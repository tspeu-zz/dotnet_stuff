                ...
                PRINT '------------ FACULTAD -----'  + @FACULTY_VALUE 
                        ...
                        BEGIN   
                                ...
                                SELECT  cd.id
                                        ...
                                        ,CASE @VALUE = 'ES' AND @VALUE = 'PT'
                                            THEN
                                            PRINT 'ES EUROS'
                                                WHEN ct.SeoUrl = 'curso-universitario'
                                                THEN                                    
                                        ...
                                        END
                                        AS NEW_TotalAmount_EU

                                            ELSE
                                            PRINT 'ES LATAM'
                                            CASE 
                                                WHEN ct.SeoUrl = 'curso-universitario' OR  ct.SeoUrl = 'diplomado'
                                                THEN                                    
                                                (SELECT round( (pa.Amount * pfs.Value )  
                                                        * (SELECT (pc.[Value] * pf.[Value]) 
                                                        FROM ProductCountryModifiers pc 
...
END
----------------------------------------------
                                               /*        ,pfs.FacultyUrl, @CategoryName , c.code                  
                                                ,pa.Amount * pfs.Value  AS AmountProduct
                                                ,( SELECT   pc.[Value] * pf.[Value] 
                                                        FROM ProductCountryModifiers pc 
                                                        INNER JOIN ProductCountryFacultyModifiers AS pf on pf.CountryCode = pc.CountryCode
                                                        WHERE pc.CountryCode = @CountryCode
                                                        AND pf.CategoryUrl = @FacultyName
                                                        AND pc.CategoryUrl =  @CategoryName ) As CountryModifier
                                        */    
/*        
